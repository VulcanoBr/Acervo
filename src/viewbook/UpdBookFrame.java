...
public class UpdBookFrame  extends JDialog {
	...
	public  UpdBookFrame (JFrame frame) {
		super(frame);
		...
		setModal(true);
		// setResizable(false).;
		setTitle("Arualizar Acervo de Livros");
	...
	protected  void buildBookFrame() {
			...
			public void keyPressed (KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					if ( codigoCatalogoField.getText() == null || codigoCatalogoField.getText().trim().isEmpty())  {
						JOptionPane.showMessageDialog(UpdBookFrame.this,
				 			...
				 			if (acervoBook == null)  {
				 					JOptionPane.showMessageDialog(codigoCatalogoField, "Codigo ISBN não existe !!!");
				 			} else {
		...
		//add(codigoBarraLabel, new GBC(0,7).insets(15, 10, 10, 10).left());
		...
		ImageIcon cadastrar = new ImageIcon("imagens/atualizar.png");
		...
		// trata botoes da tela de alteração
		trataBotoes();
				...
				String codigo = codigoCatalogoField.getText(); 
			 	if ( codigo == null || codigo.trim().isEmpty())  {
			 		JOptionPane.showMessageDialog(UpdBookFrame.this,
			 			...
			 			ConsultBookDAO.update(getAcervoBook());
			 			UpdateBookItem();
			 			JOptionPane.showMessageDialog(UpdBookFrame.this,
			 					"Acervo atualizado com sucesso.");
			 			UpdBookFrame.this.dispose();
	...
	protected AcervoBook getAcervoBook() {
		...
		Integer anoPrimeiraEdicao = Integer.parseInt((String) anoPrimEdicaoField.getText());
		Integer anoLancamento = Integer.parseInt((String) anoLancamentoField.getText());
	...
	private void UpdateBookItem () {
		for(int i=0; i<table.getRowCount(); i++){  
			...
			if (vazia > 0) {
				...
				if (semItem == 2) {
					ConsultBookItemDAO.update(acervoBookItem);  // atualiza dados
					} else {
						ConsultBookItemDAO.insert(acervoBookItem); // insere dados
					 		}
				}
		}
